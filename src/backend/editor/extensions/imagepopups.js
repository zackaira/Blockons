...
const blockonsAddEditorImgPopupAttributes = createHigherOrderComponent(
	(BlockListBlock) => {
		return (props) => {
			...
			const newClasses = blockonsPopupEnabled ? `blockons-img-popup blcks-${blockonsPopupIcon} blcks-${blockonsPopupIconPos} blcks-${blockonsPopupIconColor} ${isPremium && blockonsGalleryId ? "gallery" : "single"}` : "";
			const globalClass = isPremium && blockonsGlobalDisable ? "global-off" : "";
			const newClassnames = classnames(className, newClasses, globalClass);

			const newProps = blockonsPopupEnabled && isPremium && blockonsGalleryId !== "" ? { "data-gallery": blockonsGalleryId } : {};
...
const blockonsAddFrontendImgPopupAttributes = (
	...
	const newClasses = blockonsPopupEnabled ? `blockons-img-popup blcks-${blockonsPopupIcon} blcks-${blockonsPopupIconPos} blcks-${blockonsPopupIconColor} ${isPremium && blockonsGalleryId ? "gallery" : "single"}` : "";
	const globalClass = isPremium && blockonsGlobalDisable ? "global-off" : "";
	const newClassnames = classnames(extraProps.className, newClasses, globalClass);
