			...
			<InspectorControls>
				<PanelBody title={__('Table of Contents Settings', 'blockons')}>
					<ToggleControl
						...
						checked={canCollapse}
						onChange={(newValue) =>
							setAttributes({ canCollapse: newValue })
						}
						help={
							canCollapse
								? __(
										'The Table of headings will now collapse when not selected.',
										'blockons',
									)
								: ''
						}
				...
				<PanelBody
					title={__('Table of Contents Design', 'blockons')}
					initialOpen={false}
				>
					<SelectControl
						...
						options={[
							{
								label: __('Plain Border', 'blockons'),
								value: 'one',
							},
							{
								label: __('Dashed Border', 'blockons'),
								value: 'two',
							},
							{
								label: __('No Border', 'blockons'),
								value: 'three',
							},
						]}
						onChange={(newValue) =>
					...
					<RangeControl
						...
						value={paddingHoriz}
						onChange={(newValue) =>
							setAttributes({ paddingHoriz: newValue })
						}
						min={0}
						max={80}
						__next40pxDefaultSize={true}
					...
					<RangeControl
						...
						value={paddingVert}
						onChange={(newValue) =>
							setAttributes({ paddingVert: newValue })
						}
						min={0}
						max={80}
						__next40pxDefaultSize={true}
					...
					{showNumbers && (
						<>
							<RangeControl
								...
								value={numberSize}
								onChange={(newValue) =>
									setAttributes({ numberSize: newValue })
								}
								min={10}
								max={84}
								__next40pxDefaultSize={true}
							...
							<RangeControl
								...
								value={numberSpacing}
								onChange={(newValue) =>
									setAttributes({ numberSpacing: newValue })
								}
								min={0}
								max={100}
								__next40pxDefaultSize={true}
					...
					<RangeControl
						...
						value={paddingIn}
						onChange={(newValue) =>
							setAttributes({ paddingIn: newValue })
						}
						min={0}
						max={50}
						__next40pxDefaultSize={true}
					...
					<RangeControl
						...
						value={spacing}
						onChange={(newValue) =>
							setAttributes({ spacing: newValue ?? 10 })
						}
						min={0}
						max={50}
						__next40pxDefaultSize={true}
					...
					{showNumbers && (
						<BlockonsColorpicker
							...
							value={numberColor}
							onChange={(newValue) =>
								setAttributes({ numberColor: newValue })
							}
							paletteColors={colorPickerPalette}
						/>
					)}
					<BlockonsColorpicker
						...
						value={headColor}
						onChange={(newValue) =>
							setAttributes({ headColor: newValue })
						}
						paletteColors={colorPickerPalette}
					/>
					<RangeControl
						...
						value={headSize}
						onChange={(newValue) =>
							setAttributes({ headSize: newValue })
						}
						min={10}
						max={48}
						__next40pxDefaultSize={true}
					...
					{(showDescTop || showDescBottom) && (
						<>
							...
							<RangeControl
								...
								value={descSize}
								onChange={(newValue) =>
									setAttributes({ descSize: newValue })
								}
								min={10}
								max={48}
								__next40pxDefaultSize={true}
					...
					{showBlurbs && (
						<>
							<BlockonsColorpicker
								...
								value={blurbColor}
								onChange={(newValue) =>
									setAttributes({ blurbColor: newValue })
								}
								paletteColors={colorPickerPalette}
							/>
							<RangeControl
								...
								value={blurbSize}
								onChange={(newValue) =>
									setAttributes({ blurbSize: newValue })
								}
								min={10}
								max={48}
								__next40pxDefaultSize={true}
				...
				{isPremium && (
					<PanelBody
						title={__('Table of Contents Links', 'blockons')}
						initialOpen={false}
					>
						<ToggleControl
						...
						<p>
							{__(
								"This link functionality works only on the website's frontend, so please make sure to enable it and test it directly on the live page.",
								'blockons',
							)}
						</p>
					...
					<div className="header-txt">
						...
						{showDescTop && (
							<RichText
								...
								onChange={(newValue) =>
									setAttributes({ descTop: newValue })
								}
								allowedFormats={['core/bold', 'core/italic']}
								placeholder={__(
					...
					<ul className="blockons-toc-ul">
						{headings.map((heading, index) => (
							...
							>
								{showNumbers && (
									<div
										className="blockons-toc-number"
										style={{
											...(numberSpacing !== 8
												? {
														marginRight: `${numberSpacing}px`,
													}
												: {}),
											...(numberSize !== 18
												? {
					...
					{showDescBottom && (
						<div className="footer">
							<RichText
								...
								onChange={(newValue) =>
									setAttributes({ descBottom: newValue })
								}
								allowedFormats={['core/bold', 'core/italic']}
								placeholder={__(
									'Click the relevant heading to jump to that section.',
