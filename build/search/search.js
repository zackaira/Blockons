...
document.addEventListener("DOMContentLoaded", function () {
  const blockSearches = document.querySelectorAll(".blockons-search-block.popup");
  ...
  const searchOverlays = document.querySelectorAll(".blockons-search-popup-overlay");
  ...
  const searchCloses = document.querySelectorAll(".blockons-search-popup .blockons-close");

  if (searchCloses) {
    searchCloses.forEach(overlayItem => {
      overlayItem.addEventListener("click", () => {
        overlayItem.closest(".wp-block-blockons-search").classList.remove("blockons-show");
      });
    });
  }
});
